    ...
    protected function getUser()
    {
        ...
        if($this->module->userIdSource === 'session'){
            ...
            if(isset($session[$userIdName]) && $session[$userIdName] !== ''){
                ...
                throw new CException(Yii::t('FamitsuCodeModule.Exception', "Can not get userId by {userIdName} from session.", array('{userIdName}'=>$userIdName)));
            }
        }elseif($this->module->userIdSource === 'get'){
            if(isset($_GET[$userIdName]) && $_GET[$userIdName] !== ''){
                $paramUserId = $_GET[$userIdName];
            }else{
                throw new CException(Yii::t('FamitsuCodeModule.Exception', 'Can not get userId by {userIdName from $_GET.', array('{userIdName}'=>$userIdName)));
            }
        }else{
            throw new CException(Yii::t('FamitsuCodeModule.Exception', 'Do not know how to get userId'));
        }
