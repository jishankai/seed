<?php
/**
 * Class SeedApurifanUser is the model implements IApurifanUser to stand for the user applying famitsu code.
 * 
 ...
 * resident world.
 */
class SeedApurifanUser extends CModel implements IApurifanUser
{
    private $_playerId;
    private $_userId;
    ...
    public function receiveRewards()
    {
        ...
        foreach($this->rewards as $itemId=>$num){
            ...
            $item->addItem($itemMeta, 'apurifanCode', $num);
        }
    }
    ...
    public function applyCode($code)
    {
        ...
        try{
            $success = $code->apply();
            if(!$success){
                throw new CException(Yii::t('ApurifanCodeModule.Exception', "Failed to apply code"));
            }
    ...
    public function getAppliedCodeNum()
    {
        if(!isset($this->_appliedCodeNum)){
            $command = Yii::app()->db->createCommand("SELECT count(*) FROM apurifanCode WHERE userId = :userId AND deleteFlag = 0");
