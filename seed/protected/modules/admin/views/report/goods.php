...
<?php echo $form->errorSummary($model); ?>
    <table class="t_report1">
        ...
        <tr>
            <td class="in_title">GOODS范围</td>
            <td colspan="8">
                <?php echo $form->radioButtonList($model, 'searchGoodsRange', array(0 => '所有', 1 => 'goodsId'), array('separator' => '&nbsp;&nbsp;&nbsp;&nbsp;')); ?>
                &nbsp;
                <?php echo $form->textField($model,'searchGoodsId', array('size' => "10px")); ?>
            </td>
        </tr>
        <tr>
...
<?php if (isset($data['result'])) { ?>
    ...
    <table width="760px">
        ...
        <?php foreach ($data['result'] as $line) { 
                if (($line['num'] == 0) /*or ($line['goodsId'] == 0)*/) {
                    continue;
                }
        ?>
        <tr style="text-align:center;">
            <td><?php if (isset($line['timeStr'])) {echo $line['timeStr'];} else {echo '&nbsp;';} ?></td>
            <td><?php if (isset($line['linkId'])) {echo $line['linkId'];} else {echo '&nbsp;';} ?></td>
			<td><?php if (isset($line['linkId'])) {echo $data['name'][$line['linkId']];} else {echo '&nbsp;';} ?></td>
            <td><?php echo $line['num']; ?></td>
            <td><?php echo 0 - $line['gold']; ?></td>
        </tr>
        <?php } ?>
